...
def channel_connector(params,q_book,q_execution,q_user_order):
    if params.backtest:
        t1=Thread(target=virtual_channel_execution_details_cash,args=(q_execution,))
        t2=Thread(target=virtual_channel_price_ladders,args=(q_book,))
        #t3=Thread(target=virtual_channel_user_order,args=(q_user_order,))
    else:
...
class Paras:
    ...
    # バックテスト用パラメータ
    backtest:bool=True
    start_timestamp:int=datetime.datetime.strptime('2021040922','%Y%m%d%h').timestamp()
    end_timestamp:int=datetime.datetime.strptime('2021041000','%Y%m%d%h').timestamp()
    execute_ratio:float=0.5
